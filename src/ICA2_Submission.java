...
public class ICA2_Submission {
	    ...
	    /																							  |
	   / 		Date since last modification:	28/7/2017  -  1.45am								  |
	  /_______________________________________________________________________________________________|
	
	
	 _______________________________________________________________________________________________________________________________________
	|					                                                     |																|
	|   1      The Main Program:  System to track all payment transactions   |        Calleees: 	displayOption()							|
	| 							  and begin the ride for FunIT theme park  	 |						getOption()								|
	|																		 |						addingRidePlayersInterface()		    |
	|																		 |						addingRidePlayersInterfaceVer2()        |
	|							  Option   1: Add players					 |______________________________________________________________|
	...
	public static void main (String[] args)
	{
		...
		int lengthOfString;
		int numOfTransactions = 0;													// One Transaction is all payment for one round.
...
/*Option: 3*/		else if ( inputOption == 3)
					{
						if (numOfTransactions < 1000)
						{
							...
							for (int y = 0; y<personalCounterForTransactionDisplayPerRound; y++)												// Displays the transactions for the round.
								System.out.println(transactionDisplayPerRound[y]);																// I could empty this array after being used but the 
																																				// program will be less efficient and there is no difference.
							...
							System.out.println("\n");
							numOfPlayers = 0;
							personalCounterForTransactionDisplayPerRound = 0;																	// Resets the personal variable for transactionDisplayPerRound[y].
						}
						else 
						{
...
/*Option: -99*/				
				if (numOfPlayers > 0)																											// Disables option(-99) until no more players.
				{
	...
	|                        						 |											   |
	|   1.1     The display:  Displays the options   |        Callers: main (String[] args)        |
	...
	|														 |											   |
	|   1.2     The display:  User inputs: 1, 2, 3 or -99.   |        Callers: main (String[] args)        |
	...
	|														                               											|											  |												   |
	|   2     Adding players:  The interface / The player's particulars / The total players /   									|        Callers: main (String[] args)        |        Callees: getFirstPlayerName()           |
	|                          The price     / The discounted price 	/ The payment   	/   									|_____________________________________________|					getFirstPlayerAge()            |
	| 						   The adding of transactions                                   									    |											  |					getSecondPlayerName()          |
	|						   ( The list briefly describes the process within "Adding players" accordingly step by step,       )   |      										  |					getSecondPlayerAge()           |
	| 						   ( and briefly explains the functions of the list of variables listed below step by step as well. )   |											  |					discountDisplay()              | 
	|																															    |											  |					getDiscountsAvailable()        |
	| 						   Returns value of a part of transaction, and "number of players" indirectly.       				    |											  |________________________________________________|
...
/*Adult Age*/						if ( (playerAdultAge < 16 || playerAdultAge > 80) && playerAdultAge != -99)								// The "Adult" is too young or old to join.
							...
							if (playerAdultAge >= 16 && playerAdultAge <= 80)						// Valid inputs.
							{
								...
								System.out.println("Confirming players...");						// Skip to line 472.
							}
						}
						else if (playerAge >=6 && playerAge <=12)																			// Valid inputs for first player.
						{
							++numOfChildSingle;
							++numOfPlayers6To12;
							System.out.println("Confirming player...");																		// Skip to line 472.
						}
				...
				totalNumOfPlayers = numOfChild + numOfAdult + numOfChildSingle;						// This is line 472.
							...
							{																		// Added flexibility of inputs.
								...
								{	
									switch (num1)
									{
										...
										default:    ++redo;											// Skip to line 651 if ended at default.
									...
									switch (num2)
									{
										...
										default:    ++redo;
													System.out.println("Invalid Input.");			// Skip to line 651 if ended at default.
													System.out.println("Try again.");
									}
								...
								{
									++redo;															// Error in input.
									System.out.println("Invalid Input.");							// Skip to line 651.
									System.out.println("Try again.");
								}
							}																		// Error in input.
							else
...
/*End*/	numOfPaymentMade = totalNumOfPlayers;
	...
	|                                                             |															|
	|   2.1     Adding players:  Adding the first player's name   |        Callers: addingRidePlayersInterface()			|
	|_____________________________________________________________|					addingRidePlayersInterfaceVer2()        |		
	...
	|  															 |														   |
	|   2.2     Adding players:  Adding the first player's age   |        Callers: addingRidePlayersInterface()			   |
	|____________________________________________________________|				   addingRidePlayersInterfaceVer2()        |	
	 															 |_________________________________________________________|	
	 																																																												 **/
	...
	|														           	    |														  |
	|   2.3     Adding players(Optional):  Adding the adult player's name   |        Callers: addingRidePlayersInterface()			  |															 
	|_______________________________________________________________________|				  addingRidePlayersInterfaceVer2()        |
	 																		|_________________________________________________________|
																																																													  **/
	...
	|																	   |														 |
	|   2.4     Adding players(Optional):  Adding the adult player's age   |        Callers: addingRidePlayersInterface()			 |	
	|______________________________________________________________________|				 addingRidePlayersInterfaceVer2()        |
																		   |_________________________________________________________|
																										 																																			 **/
	...
	|                                                |														   |
	|   2.5     Adding players:  Displays discounts  |        Callers: addingRidePlayersInterface()			   |	
	|________________________________________________|				   addingRidePlayersInterfaceVer2()        |
	 												 |_________________________________________________________|
																																																													 **/
	...
	|  														 |														   |
	|   2.6     Adding players: Discounts applicable input   |        Callers: addingRidePlayersInterface()			   |
	|________________________________________________________|				   addingRidePlayersInterfaceVer2()        |			
															 |_________________________________________________________|	
																										 																																			 **/
	...
	|														                               	 	 |											   |												|
	|   2-2     Adding players: (For when the current players added is already 4 players    	 |        Callers: main (String[] args)        |        Callees: getFirstPlayerName()           |
	| 							 and a child + accompanying adult is not allowed)           	 |_____________________________________________|				 getFirstPlayerAge()			|
	| 							  																 |											   |				 discountDisplay()				|
	| 							The interface / The player's particulars / The total players /   |       									   |				 getDiscountsAvailable()        |
	|                           The price     / The discounted price 	 / The payment   	 /   |											   |________________________________________________|
	...
	public static String addingRidePlayersInterfaceVer2()
	{
		...
		String playerName;						int playerAge 			= 0;										// To prevent over commenting:
																													// Comments will be kept at a minimum here since it is literally a repeat in codes from original version, 
		int numOfAdult 			= 0;     		int numOfChildSingle 	= 0;										// except with no adding of second player.
																										
		int numOfPlayers6To12 	= 0;    		int numOfPlayers13To16 	= 0;										// Refer to above comments in "Section 2" if needed.
...
/*Price*/	totalPrice = totalPrice-(discount1*totalPrice*pOSBDiscountRate);
			totalPrice = totalPrice-(discount2*totalPrice*oCBCDiscountRate);
			totalPrice = totalPrice-(discount3*totalPrice*nYPStudentDiscountRate);
			totalPrice = totalPrice-(discount4*totalPrice*safraCardDiscountRate);
